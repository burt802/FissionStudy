% "Fission as a source of variation..." Example 4
epsilon = 10^-10; e0 = .0001; e1 = 1.5;  
b0 = .5; a = 1.05; s = .05;   
f0 = .0005; Msigma = .01; mutprob = .05; 
mu = .000; d = .002; r = 0;  
FissBins = 20;
MaxG = 1000; grid = 100; Xmin = 0; Xmax = 1; dx = (Xmax-Xmin)/grid;
...
for i = 1:Groups                        % initialize groups
    state(i,.45*grid:.55*grid) = 200;
    alive(i) = 1; pop(i) = sum(state(i,:))*dx;
end
T = 20000; dt = .1; t = 0; iter = 0; fisstot = 0;
while t < T
    t = t + dt; iter = iter + 1;
    if floor(iter/200) == iter/200
        k = iter/200; time(k) = t;
    ...
    for i = 1:last             % update group i state
        if alive(i) == 1 
            y = sum(state(i,:).*M123)*dx/sum(state(i,:)); %group i type
            for ii = 1:grid  % total birth rate for each type
                x = ii/grid;
                birthrate(ii) = b0 * (1 + s*(a*y - x));
            end
            birthrate = birthrate .* state(i,:) * dx;
            deathrate = state(i,:) * d * pop(i) * dx; %total deathrate for each type
            for ii = 1:grid
                new(ii) = 0;      
                for j = max(1,ii-mutrange):min(grid,ii+mutrange) % total birthrate due to j + mutation
                    if j == ii
                        new(ii) = new(ii) + birthrate(ii)*(1-mutprob + mutprob*mutdist0);
                    else
                        new(ii) = new(ii) + birthrate(j)*mutprob*mutdist(abs(ii-j));
                    end
                end
                Z(1,ii) = (new(ii) - deathrate(ii))/dx;
                Z(2,ii) = (new(ii) + deathrate(ii))/dx;
            end
            state(i,:) = state(i,:) + Z(1,:)*dt; % baseline case, i.e., no stochastic term
            %state(i,:) = max(0,state(i,:) + Z(1,:) * dt + sqrt(Z(2,:)*dt).*randn(1,grid)); 
            for j = 1:grid
            ...
            if rand < e0*exp(-e1*y) * Groups  % Example 4
