% "Fission as a source of variation..." Ex3
% along with HybridRate4.m with x0 = .5, b0 = .25, sigmab = 3
epsilon = 10^-10; e0 = .00005; e1 = 5; 
f0 = .0005; Msigma = .01; mutprob = .1; d = .001; r = 0;
FissBins = 20;
...
while t < T
    ...
    for i = 1:last
        if alive(i) == 1
            pop(i) = sum(state(i,:))*dx; % find Ave, Var for each group
    ...
    for i = 1:last             % update group i state
        if alive(i) == 1 
            ...
            if rand < e0*exp(-e1*VarCoopLevel(i)) * Groups  % Example 3
