% "Fission as a source of variation..." Example 1
% uses HybridRate4.m with x0 = .5; b0 = .1; sigmab = .3;
epsilon = 10^-10; e0 = .00005; e1 = .00003;
f0 = .0005; Msigma = .1; mutprob = .1; 
mu = .000; d = .0001; r = .6;
FissBins = 20;
...
while t < T
    ...
    if floor(iter/250) == iter/250
        ...
        [t/T, r, Groups, betweenGroupAve, sqrt(betweenGroupVar2), EVarCoopLevel(k)] %E(Y), sd(E(Y|G)), E(Var(Y|G))
    end
    pop = zeros(1,MaxG); 
    ...
    for i = 1:last             % update group i state
        if alive(i) == 1 
            ...
            if rand < e0*Groups   % Example 1          does i die?
